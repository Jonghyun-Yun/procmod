...
**** collocation
#+begin_src R :var out_dir="tickets/" :results none
tt = readr::read_tsv(paste0(out_dir,"bi_ss_class_4.tsv"), col_names=F, progress = F)
names(tt) = c("bigram","likelihood_ratio")
#+end_src

#+begin_src R :results html :exports results
knitr::kable(tt)
#+end_src
...
#+begin_src R :var out_dir="tickets/" :results none
tt = readr::read_tsv(paste0(out_dir,"bi_ss_class_3.tsv"), col_names=F, progress = F)
names(tt) = c("bigram","likelihood_ratio")
#+end_src
...
#+begin_src R :var out_dir="tickets/" :results none
tt = readr::read_tsv(paste0(out_dir,"bi_ss_class_2.tsv"), col_names=F, progress = F)
names(tt) = c("bigram","likelihood_ratio")
#+end_src
...
#+begin_src R :var out_dir="tickets/" :results none
tt = readr::read_tsv(paste0(out_dir,"bi_ss_class_1.tsv"), col_names=F, progress = F)
names(tt) = c("bigram","likelihood_ratio")
#+end_src
...
:PROPERTIES:
:EXPORT_FILE_NAME: _index
:EXPORT_HUGO_BUNDLE: Party Inviations
:END:
**** collocation
#+begin_src R :var out_dir="party_invitations-1/" :results html
class = "4"
for (class in c(4,3,2,1) {
...
#+RESULTS:
#+begin_export html
Error: unexpected '{' in "for (class in c(4,3,2,1) {"
## Class  4
