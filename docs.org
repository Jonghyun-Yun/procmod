...
A test-taker were asked to book tickets for a football game. S/he needs to gather information from calendars to pick an optimal date.
xx unique actions,
On average, a repondent has a action sequence of length xxx actions, and spend xxx seconds overall.

** Model
Let $S$ denotes a set of all possible action. For each state $m \in S$, $A_{m}$ denotes a set of competing transitions $\{l_1, \ldots, l_{n_m}\}$ that can be taken directly after $m$.
Let \(Y_k(t)\) denote an action of k-th respondent at time $t$. The $k$-th respondent takes a sequence of $M_{k}$ actions. We define \(  \delta_{k,n,m} = 1 \) if person k's n-th action is m; 0 otherwise. Thus, \( \delta_{k,n,m}  \delta_{k,n+1,l} = 1 \) menas person k's n-th transition ($n < M_{k}$) is m to l.
...
negative sampling
cosine similarity
[[id:9aad4ce5-7a81-436c-9f59-155d4cd47279][Word2Vec]]
** Multistate model
The intensity function $q_{ml}(\cdot)$ represents the instantaneous risk of moving from action $m$ to $l$:
\begin{align*}
q_{ml}\left(t ; \mathcal{F}_{t}\right)= & \lim _{\delta t \rightarrow 0} \frac{P\left(Y(t+\delta t)=l \mid Y(t)=m, \mathcal{F}_{t}\right)}{\delta t}, m \neq l, m, l \in S,
...
** data structure
https://www.rdocumentation.org/packages/msm/versions/1.6.8/topics/msm2Surv
Given a configured transition matrix, \texttt{msm} package transform data to a desired "long" format:
...
- [[skim:///Users/yunj/Zotero/storage/9D6G7MNF/gelman_bayesian_2014.pdf::591;;1][Continuous distributions org-id:{5c29e214-f86d-41d5-89a0-e164602bf6b8}:org-id]]
*** update \(\kappa_{m}\):
For each $k$, we draw \(\kappa_{m}^{* }\)$ from a symmetric MH jumping distribution, and accept $\kappa_{m}^{(l)} = \kappa_{m}^{* }$ with probability $\min(1, r_{{\kappa_{m}}^{* }})$ where
