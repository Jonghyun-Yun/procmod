...
Probem Solving in Technology-Rich Environments (PS-TRE)
#+begin_quote
...
word embedding: to substitute a symbolic representation with a vectoric representation of words
Each action sequence has different
...
Skip-gram modeling of the above form coupled with negative sampling is often referred to as one of the word2vec models cite:mikolov_distributed_2013. A second proposed model of word2vec is the continuous bag-of-words model (CBOW), which predicts a word from the context—in reverse from the skip-gram model.
...
:END:
For each $m$, we draw \(\kappa_{m}^{* }\)$ from a symmetric MH jumping distribution, and accept $\kappa_{m}^{(l)} = \kappa_{m}^{* }$ with probability $\min(1, r_{{\kappa_{m}}^{* }})$ where

\[
  \begin{align*}
\log r_{{\kappa_{m}}^{* }} =& \sum_{k=1}^K \sum_{n = 1} ^{M_{k}} \left[  \delta_{k,n,m} (\log \kappa_{m}^{* } - \log \kappa_{m}^{(l-1)})-\sum_{l \in A_{m}} dt_{k,n} (\kappa_{m}^{* } - \kappa_{m}^{(l-1)}) \tau_{k} e^{(\theta_{k} + \beta) D_{ml} } + \log \frac{\pi(\kappa_{m}^{* })}{\pi(\kappa_{m}^{t})} \right].
\end{align*}
\]
...
[[file:src/update_kappa.cpp::// std::cout << "tau: " <<tau(sid.at(ii)) << "\n";][code: \kappa update]]
...
#+end_src
*** update \(\tau_{k}\)
For each $m$, we draw \(\tau_k^{* }\)$ from a symmetric MH jumping distribution, and accept $\tau_{m}^{(l)} = \tau_{m}^{* }$ with probability $\min(1, r_{{\tau_{m}}^{* }})$ where

\begin{align*}
\log r_{{\tau_{k}}^*} = \sum_{m \in S} \sum_{n=1}^{M_{k}} \left[  \delta_{k,n,m} (\log \tau_{k}^* - \log \tau_{k}^{(l-1)})- \sum_{l \in A_m} dt_{k,n} \kappa_{m}e^{(\theta_{k} + \beta) D_{ml}} ( \tau_{k}^* -  \tau_{k}^{(l-1)} )
+ \log \frac{\pi(\tau_{k}^*)}{\pi(\theta_{k}^{t})}\right].
\end{align*}
...
[[file:src/update_tau.cpp::post_b +=][code: \tau update]]
...
\begin{align*}
\log r_{{\theta_{k}}^{* }} =& \sum_{m \in S} \sum_{n=1}^{M_{k}} \left[ \delta_{k,n,m} (\theta_{k}^{* } - \theta_{k}^{(l-1)})D_{ml} -\sum_{l \in A_m} dt_{k,n} \kappa_{m} \tau_{k} e^{ \beta D_{ml} }(e^{\theta_{k}^{* }D_{ml}} -  e^{\theta_{k}^{(l-1)} D_{ml} })
+ \log \frac{\pi(\theta_{k}^{* })}{\pi(\theta_{k}^{(l-1)})} \right].\\
\end{align*}
