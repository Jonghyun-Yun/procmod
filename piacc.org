...
\pi\left(\beta_{k} | \sigma_{\beta}^{2}\right) & \sim \mathrm{N}\left(\mu_{\beta}, \sigma_{\beta}^{2}\right); \\
\pi\left(\mu_{\beta}) ~
...
&-\sum dt \kappa_{m} \tau_{k} e^{ \theta_k D_{ml} }(e^{\beta_{k}^{* }D_{ml}} -  e^{\beta_{k}^{(l-1)} D_{ml} })
...
:PROPERTIES:
:header-args:R: :tangle preproc-data.r
:END:
...
#+begin_src R
email = email %>% filter(word !="START")
state = unique(email$word)
ntate = numeric(nrow(email))
ii = 0
for (ww in email$word) {
  ii = ii + 1
...
#+RESULTS:
: Error in email %>% filter(word != "START") :
:   could not find function "%>%"
: Error in unique(email$word) : object 'email' not found
: Error in nrow(email) : object 'email' not found
: Error in email : object 'email' not found
: Error: object 'ntate' not found

#+begin_src R
uid = unique(email$SEQID)
pid = numeric(nrow(email))
ii = 0
for (nn in email$SEQID) {
  ii = ii + 1
  pid[ii] = which(uid == nn)
  }
email$pid = pid
...
#+begin_src R
metadata = readr::read_tsv("metadata.tsv", col_names = FALSE)
vectors = readr::read_tsv("vectors.tsv", col_names = FALSE)
#+end_src
...
** write data
:PROPERTIES:
:header-args:R: :tangle write-data.R
:END:

#+begin_src R :results none
library(msm)
df = email %>% select(pid, timestamp, response, state)
dat <- msm2Surv(data=df, subject="pid", time="timestamp", state="state",
         Q=Q)
##dat
## attr(dat, "trans")
...
dat$dist = foreach (i = 1:nrow(dat), .combine="c") %dopar% Dml[dat$from[i], dat$to[i]]
#+end_src
...
** init
:PROPERTIES:
:header-args:R: :tangle write-init.r
:END:
#+begin_src R
kappa = rep(0.1, M)
tau = rep(0.1, N)
...
mu_beta = matrix(0.0,1,1)
sigma_beta = matrix(1.0,1,1)
...
#+RESULTS:
: Error in cbind(M, N, dq, dn, dc) : object 'M' not found

#+begin_src R
readr::write_csv(as.data.frame(cbind(a_kappa,b_kappa,jump_kappa)),"input/hyper_kappa.csv", col_names = FALSE)
readr::write_csv(as.data.frame(cbind(mu_beta,sigma_beta,jump_beta)),"input/hyper_beta.csv", col_names = FALSE)
readr::write_csv(as.data.frame(cbind(mu_theta,sigma_theta,jump_theta)),"input/hyper_theta.csv", col_names = FALSE)
...
** for-loop index
:PROPERTIES:
:header-args:R: :tangle write-loop-index.r
:END:
#+begin_src R
