...
q_{ml}\left(t ; \mathcal{F}_{t}\right = &
    q_{ml} (t - t_{m}; \boldsymbol{\lambda}, \boldsymbol{\beta}, \mathbf{z}(t)) \\
...
We use the constant baseline hazard (based on current and next action):
\[
...
#+RESULTS:
| csv | jpg | jpeg | tiff | png | svg | gif | mp4 | pdf | odt | doc | ppt | xls | docx | pptx | xlsx |
