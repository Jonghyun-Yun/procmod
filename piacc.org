...
\]
where \(A_{m}\) denotes a set of possible actions that can taken directly after action $m$, and
\(dt_{k,n}\) denotes $t_{k,n}^{stop} - t_{k,n}^{start}$.
- large $\kappa$: fast out-of-state action
- large $\tau$: fast action transition
  
Proportional hazard term:
\[
  e^{\theta_{k} D_{ml} }
\]
- \(D_{ml}\): cosine similarity([-1,1]) between actions $m$ and $l$.
...
hazard rate:
\[
q_{ml}(dt) = \lambda_{ml}(dt_{k,n}) e^{\theta_{k} D_{ml} }
\]
** likelihood
\[
