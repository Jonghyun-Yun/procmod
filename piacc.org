...
\[
  \lambda_{ml}(t) = \lambda_{m1}(t) e^{\theta_{l}} \text{ for } l \neq 1.
\]
Or
\[
  \lambda_{ml}(t) = \lambda_{m} + \lambda_{j} + \theta_{\lambda},
\]
out-of-action, item, person parameters.

Proportional hazard term:
\[
  e^{(\beta_m + \beta_k + \theta_{\beta}) D_{ml}}
\]
...
\]
\[
  likelihood =\prod_{all people} f_{ml}(t) \prod_{g \in possible actions} S_{mg}(t)
\]
** data structure
*** mstate R package
