...
where $\boldsymbol{\alpha}$ is a vector of intercepts, and $\boldsymbol{\beta}$ is coefficients associated with $\mathbf{z}(t)$, $\lambda_{k,m\rightarrow l}(t)$ is a baseline intensity function. For each state $l$, there are competing transitions $m_1, \ldots, m_{n_l}$. This mean there are $n_{l}$ corresponding survival models for state $l$, and overall $K=\sum_l n_l$ models. Models with no shared parameters can be estimated separately.
* Model V0.2
Let $S$ denotes a set of all possible action. For each state $m \in S$, $A_{m}$ denotes a set of competing transitions $\{l_1, \ldots, l_{n_m}\}$ directly after $m$.
Let \(Y_k(t)\) denote an action of k-th respondent at time $t$. All respondents begin problem solving processes at time $t=0$.
...
\[
q_{ml}\left(t ; \mathcal{F}_{t}\right)= q_{ml} \left(t - t_m; z(t)? \right)\]
...
The time-inhomogeneous Markovian leads to
\[
  P(Y_k(t + dt) = l | Y_k(t) =m) = p_{ml,k}(dt),
\]
for all $m \in S$, $l \in A_{m}$, and $t>0$.

The intensity function $q_{ml}(\cdot)$ represents the instantaneous risk of moving from action $m \in S$ to $l \in A_{m}$:
\begin{align*}
  q_{ml} (t ; \boldsymbol{\lambda}, \boldsymbol{\beta}, \mathbf{z}(t)) = & \lambda_{ml}(t)  e^{(\boldsymbol{\beta}' \mathbf{z}(t) +  \theta_{k}) D_{ml}},
\end{align*}
...
Action transition is assumed to follow Semi-Markovian (time-inhomogeneous Markovian), which means the intensity depends on the sojourn time (time spent on the current action). This is often called "clock reset" approach as opposed to "clock forward" approach. Let $dt$ denote the sojourn time.

We use the constant baseline hazard:
\[
  \lambda_{ml}(dt) = \kappa_{m} \tau_{k} \text{ for } l \in A_{m}.
\]

baseline (based on current and next action)
\[
  \lambda_{ml}(dt_{k,n}) = \kappa_{m} \tau_{k} \text{ for } l \in A_{m}/\]

A running model has no coviarate terms::
\[
q_{ml}(dt) = \lambda_{ml}(dt) e^{\theta_{k} D_{ml} }
\]
...
\[
  S_{ml}(dt) = e^{-\int_{0}^{dt} q_{ml}(x) \dd x}
\]
...
:END:
See the codebook! [[file:data/PIAAC_cleaned_data_1110/Problem_solving/PS_BOOKLET_ITEM.csv][PS_BOOKLET_ITEM.csv]]
#+begin_src R
